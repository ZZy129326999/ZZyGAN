...
#     "3x3 convolution with padding"
#     return nn.Conv2d(in_planes, out_planes, kernel_size=13, stride=1, padding=1, bias=False)
...
#         print(type(i), i)
#         print(type(arg_parent[i]), arg_parent[i])
...
class NEXT_STAGE_G(nn.Module):
    def __init__(self, ngf, use_hrc = 1, num_residual=cfg.GAN.R_NUM):
        ...
        if use_hrc == 1: # For parent stage
            self.ef_dim = cfg.SUPER_CATEGORIES  # 3

        else:            # For child stage
            self.ef_dim = cfg.FINE_GRAINED_CATEGORIES  # 3
