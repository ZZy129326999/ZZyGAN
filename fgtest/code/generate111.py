...
class FineGAN_evaluator(object):
    ...
    def __init__(self, dirr, name):
        ...
        self.save_dir = os.path.join('ZZYdata3/', dirr)
...
if __name__ == "__main__":
    ...
    cfg.TEST_CHILD_CLASS = 2
    cfg.TEST_BACKGROUND_CLASS = 2
    cfg.TRAIN.NET_G = '../output/Fungus2/Model/netG_23000.pth'
    cfg.TRAIN.NET_D = '../output/Fungus2/Model/netD'
        
    for i in range(4000, 10000, 3):  # 0, 1
        random.seed(i)
        torch.manual_seed(i)
        if cfg.CUDA:
...
#         from trainer import FineGAN_evaluator as evaluator
        algo = FineGAN_evaluator(classes[1], i//3 + 1 + 30000)
