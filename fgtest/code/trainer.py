...
def child_to_parent(child_c_code, classes_child, classes_parent):  # get child to parent relaitionship information
    ratio = classes_child / classes_parent  # child / parent
#       print("---classes_child type =", type(classes_child))
    arg_parent = torch.argmax(child_c_code, dim = 1) / ratio  
    parent_c_code = torch.zeros([child_c_code.size(0), classes_parent]).cuda()
    for i in range(child_c_code.size(0)):  # have relationship
...
def load_params(model, new_param):  # paras = new_paras
    with torch.no_grad():
...
def define_optimizers(netG, netsD):  # G, D Adam optimize  3x2
    optimizersD = []
    num_Ds = len(netsD)
    ...
    optimizerG = []
    optimizerG.append(optim.Adam(netG.parameters(), lr=cfg.TRAIN.GENERATOR_LR, betas=(0.5, 0.999)))

    for i in range(num_Ds):  # 3 D 
        if i==1:   
                opt = optim.Adam(netsD[i].parameters(), lr=cfg.TRAIN.GENERATOR_LR, betas=(0.5, 0.999))
                optimizerG.append(opt)
        elif i==2:
                opt = optim.Adam([{'params':netsD[i].module.jointConv.parameters()},{'params':netsD[i].module.logits.parameters()}], lr=cfg.TRAIN.GENERATOR_LR, betas=(0.5, 0.999))
                optimizerG.append(opt)
...
def save_model(netG, avg_param_G, netsD, epoch, model_dir):
    ...
    for i in range(len(netsD)):
        ...
        torch.save(
            netD.state_dict(),
            '%s/netD_%d.pth' % (model_dir, i))
    print('Save G/Ds models.')


def save_img_results(imgs_tcpu, fake_imgs, num_imgs,
                     count, image_dir, summary_writer):
    num = cfg.TRAIN.VIS_COUNT  # 64

    real_img = imgs_tcpu[-1][0:num]
    vutils.save_image(real_img, '%s/real%09d.png' % (image_dir,count), normalize=True)  # pixels is [0, 1] 
    real_img_set = vutils.make_grid(real_img).numpy()
    real_img_set = np.transpose(real_img_set, (1, 2, 0))
    real_img_set = real_img_set * 255  # get pixels  [0,1]x255 
    real_img_set = real_img_set.astype(np.uint8) # conserve memory
    ...
    def train_Dnet(self, idx, count):
      if idx == 0 or idx == 2: # Discriminator is only trained in background and child stage. (NOT in parent stage)
        ...
        if idx == 0: # Background stage
            ...
            errD_real_uncond = torch.mul(errD_real_uncond, weights_real)  # Masking output units which correspond to receptive fields which lie within the bounding box
...
#         print("errD------->", type(errD))  # add zzy
        
        if (idx == 0 or idx == 2):
            errD.requires_grad_(True)  # add zzy
            errD.backward()
            optD.step()
...
#         errG_total = 0.   # add zzy
#         errG_total = Variable(torch.tensor(errG_total, requires_grad=True), requires_grad=True)
        errG_total = 0
        flag = count % 100
        batch_size = self.real_fimgs[0].size(0)
        ...
        for i in range(self.num_Ds):
            ...
            if i == 0 or i == 2:  # real/fake loss for background (0) and child (2) stage
                real_labels = torch.ones_like(outputs[1]).cuda()
            ...
            if i == 1: # Mutual information loss for the parent stage (1)
                    ...
                    errG_info = criterion_class(pred_p[0], torch.nonzero(p_code.long().cuda())[:,1])
            elif i == 2: # Mutual information loss for the child stage (2)
                    pred_c = self.netsD[i](self.fg_mk[i-1])
                    errG_info = criterion_class(pred_c[0], torch.nonzero(c_code.long().cuda())[:,1])
    ...
    def train(self):
        ...
        hard_noise = Variable(torch.FloatTensor(self.batch_size, nz).normal_(0, 1)).cuda()
        ...
        print ("Starting normal FDGAN training..")
        count = start_count
        ...
        for epoch in range(start_epoch, self.max_epoch):
            ...
            for step, data in enumerate(self.data_loader, 0):
                ...
                self.imgs_tcpu, self.real_fimgs, self.real_cimgs, self.c_code, self.warped_bbox = self.prepare_data(data)

                # Feedforward through Generator. Obtain stagewise fake images
                noise.data.normal_(0, 1)
                self.fake_imgs, self.fg_imgs, self.mk_imgs, self.fg_mk = self.netG(noise, self.c_code)
                ...
                errD_total = 0
                for i in range(self.num_Ds):
                  if i == 0 or i == 2: # only at backgound and child stage
                ...
                if count % cfg.TRAIN.SNAPSHOT_INTERVAL == 0:
                    ...
                    with torch.set_grad_enabled(False):
                        self.fake_imgs, self.fg_imgs, self.mk_imgs, self.fg_mk = self.netG(fixed_noise, self.c_code)
                    save_img_results(self.imgs_tcpu, (self.fake_imgs + self.fg_imgs + self.mk_imgs + self.fg_mk), self.num_Ds, count, self.image_dir, self.summary_writer)
                    self.netG.train()
        ...
        for step, data in enumerate(self.data_loader, 0):
            ...
            if (count % 100) == 0:
                print('''[%d/%d][%d]  Loss_D: %.2f Loss_G: %.2f Time: %.2fs  diffcult'''  
...
class FineGAN_evaluator(object):
    ...
    def __init__(self):
        ...
        self.save_dir = os.path.join(cfg.SAVE_DIR, 'images1')
    ...
    def evaluate_finegan(self):
            if cfg.TRAIN.NET_G == '':
                ...
                self.save_image64(fake_imgs[2][0], self.save_dir, 'child_final')
