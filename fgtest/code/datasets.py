...
def get_imgs(img_path, imsize, bbox=None, transform=None, normalize=None):
    img = Image.open(img_path).convert('RGB')  # jiazai image
    width, height = img.size  #image is height, width
    if bbox is not None:  
...
#         x2 = np.minimum(width, center_x + r)
        x1, y1, x2, y2 = bbox[0], bbox[1], bbox[2], bbox[3]  # get boundig_box addzzy
        fimg = deepcopy(img)
    ...
    retf = []
    retc = []
    re_cimg = transforms.Resize(128)(cimg)  #imsize[1]
    ...
    warped_x2 = bbox[2] * re_width / width
    warped_y2 = bbox[3] * re_height / height
...
class Dataset(data.Dataset):
    ...
    def load_bbox(self):
        ...
        bbox_path = os.path.join(data_dir, 'chair_bounding_boxes3.txt')
#         bbox_path = os.path.join(data_dir, 'bounding_boxes3.txt')
        df_bounding_boxes = pd.read_csv(bbox_path,
                                        delim_whitespace=True,
                                        header=None).astype(int)
        filepath = os.path.join(data_dir, 'chair_images3.txt')
#         filepath = os.path.join(data_dir, 'images3.txt')
        df_filenames = pd.read_csv(filepath, delim_whitespace=True, header=None, error_bad_lines=False)
...
#         print(filenames)
    ...
    def prepair_training_pairs(self, index):
        ...
        if self.bbox is not None:
            ...
            bbox = None
        data_dir = self.data_dir
        img_name = '%s/%s/%s.jpg' % (data_dir, cfg.DATASET_NAME, key[4:])
...
#         rand_class= random.sample(range(cfg.FINE_GRAINED_CATEGORIES),1); # Randomly generating child code during training
        rand_class = int(key[:3]) - 1
        c_code = torch.zeros([cfg.FINE_GRAINED_CATEGORIES,])
        c_code[rand_class] = 1  # one hot code
