...
class ImageDatasetWithIndex(ImageDataset):
    ...
    def __getitem__(self, index):
        ...
        try:
            print(img) if '.ipynb' in img else None
...
#     criterion = LabelSmoothCELoss().cuda()
    ...
    with torch.no_grad():
        ...
        for batch_idx, (input, target,idxs) in enumerate(loader):
            ...
            # compute output
            output = model(input)
            value, indices = output.topk(min(5, args.num_classes))
            ...
            if args.transfer:
                ...
                acc1, acc5 = accuracy(output.detach(), target, topk=(1, min(5, args.num_classes)))
            print("=============",loss.item(),acc1.item(),acc5.item(),input.size(0), "=================")
